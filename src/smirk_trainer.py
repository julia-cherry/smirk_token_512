...
class SmirkTrainer(BaseTrainer):
    ...
    def step1(self, batch):
        ...
        encoder_output = self.smirk_encoder(batch['img_224'], batch['img'])
        

        with torch.no_grad():
            base_output = self.base_encoder(batch['img_224'], batch['img'])
        ...
        #distangled for expression,in a batch
        '''
        swap_num = B // 2
        encoder_output_swap = {}
        gt_expression_params = torch.cat([encoder_output['expression_params'][swap_num:], encoder_output['expression_params'][:swap_num]],dim=0)
        for k,v in encoder_output.items():
            if k != 'expression_params':
                encoder_output_swap[k] = v
            else:
                encoder_output_swap[k] = gt_expression_params
        flame_output_swap = self.flame.forward(encoder_output_swap)
        renderer_output_swap = self.renderer.forward(flame_output_swap['vertices'], encoder_output_swap['cam'],
                                                landmarks_fan=flame_output_swap['landmarks_fan'], landmarks_mp=flame_output_swap['landmarks_mp'])
        rendered_img_swap = renderer_output_swap['rendered_img']
        '''
        ...
        # print(valid_landmarks.max().item())
        # print(flame_output['landmarks_fan'].max().item())
        ...
        # print('------')
        # print(encoder_output['expression_params'].shape)
        # print(flame_output['landmarks_mp'].shape)   #(bs,105,2)
        # print(batch['landmarks_mp'].shape)    #(bs,105,2)
        # exit()
        ...
        if self.config.train.use_base_model_for_regularization:
            with torch.no_grad():
                base_output = self.base_encoder(batch['img_224'])
        else:
        ...
        if self.config.arch.enable_fuse_generator:
            ...
            # print(img.shape) #512
            # print(rendered_img.shape) #512
            # print(masked_img.shape) #512
            reconstructed_img = self.smirk_generator(torch.cat([rendered_img, masked_img], dim=1), encoder_output['token'])
            ...
            if self.config.train.loss_weights['emotion_loss'] > 0:
                ...
                reconstructed_img_p = self.smirk_generator(torch.cat([rendered_img, masked_img], dim=1), encoder_output['token'])
                for param in self.smirk_generator.parameters():
    ...
    def step2(self, encoder_output, batch, batch_idx, phase='train'):
        ...
        reconstructed_img_2nd_path = self.smirk_generator(torch.cat((rendered_img_2nd_path, masked_img_2nd_path), dim=1).detach(), encoder_output['token'].detach())
        if self.config.train.freeze_generator_in_second_path:
            reconstructed_img_2nd_path = reconstructed_img_2nd_path.detach()

        
        #put the reconstrucred image into smirk_encoder
        reconstructed_img_2nd_path_224 = F.interpolate(reconstructed_img_2nd_path, size=(224, 224), mode='bilinear', align_corners=True)
        recon_feats = self.smirk_encoder(reconstructed_img_2nd_path_224.view(Ke * B, C, 224, 224), reconstructed_img_2nd_path.view(Ke * B, C, 512, 512)) 
    ...
    def step(self, batch, batch_idx, losses_AM_l1, losses_AM_per, phase='train'):
