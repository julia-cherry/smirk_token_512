...
class BaseTrainer(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        logging.basicConfig(
            filename=f'{self.config.train.log_path}/training.log',
            level=logging.INFO,
    ...
    def logging(self, batch_idx, losses, phase):
        ...
        if phase == 'train':
            if self.config.train.log_losses_every > 0 and batch_idx % self.config.train.log_losses_every == 0:
                ...
                for k, v in losses.items():
                    if k not in self.train_loss_history:
                        self.train_loss_history[k] = []
                    self.train_loss_history[k].append(v)
            
            if self.config.train.log_losses_every > 0 and batch_idx % self.config.train.plot_losses_every == 0 and phase == 'train':
                self.plot_losses('train')
        
        if phase == 'val':
            if self.config.train.log_losses_every > 0 :
                # print losses in one line
                loss_str = ''
                loss_str = loss_str + 'val: '
                for k, v in losses.items():
                    loss_str += f'{k}: {v:.6f} '
                print(loss_str)
                logging.info(loss_str)
                ...
                for k, v in losses.items():
                    if k not in self.val_loss_history:
                        self.val_loss_history[k] = []
                    self.val_loss_history[k].append(v)
            
            if self.config.train.log_losses_every > 0 and phase == 'val':
                self.plot_losses('val')
    
    def plot_losses(self, phase='train'):
        plt.figure(figsize=(12, 6))
        if phase == 'train':
            for k, v in self.train_loss_history.items():
                ...
                plt.legend()
                plt.grid()
                plt.savefig(f'{self.config.train.log_path}/{k}_{phase}_losses.png')
                plt.close()
        if phase == 'val':
            for k, v in self.val_loss_history.items():
                plt.plot(range(len(v)), v, label=k)
                plt.xlabel('Batch')
                plt.ylabel('Loss')
                plt.title('Validating Losses')
                plt.legend()
                plt.grid()
                plt.savefig(f'{self.config.train.log_path}/{k}_{phase}_losses.png')
                plt.close()
    ...
    def save_visualizations(self, outputs, save_path, show_landmarks=False):
        ...
        if show_landmarks:
            original_img_with_landmarks = batch_draw_keypoints(outputs['img'], outputs['landmarks_mp'], color=(0,255,0)) #绿色
            original_img_with_landmarks = batch_draw_keypoints(original_img_with_landmarks, outputs['landmarks_mp_gt'], color=(0,0,255)) #红色
            original_img_with_landmarks = batch_draw_keypoints(original_img_with_landmarks, outputs['landmarks_fan'][:,:17], color=(255,0,255)) #紫红色
        ...
        image_keys = ['img_mica', 'rendered_img_base', 'rendered_img', 
                    ...
                    'rendered_img_mica_zero', 'rendered_img_zero', 
                      'masked_1st_path', 'reconstructed_img', 'loss_img', 'reconstructed_img_swap_token', 'reconstructed_img_swap_expression', 
                      '2nd_path']
    ...
    def create_visualizations(self, batch, outputs):
        ...
        if self.config.arch.enable_fuse_generator:
            ...
            # visualizations['reconstructed_img_swap_token'] = outputs['reconstructed_img_swap_token']
            # visualizations['reconstructed_img_swap_expression'] = outputs['reconstructed_img_swap_expression']

        for key in visualizations.keys():
            visualizations[key] = visualizations[key].detach().cpu()

        # 3. MICA, MICA_LOSS = 0 in stage 2.
        if self.config.train.loss_weights['mica_loss'] > 0:
    ...
    def set_freeze_status(self, config, batch_idx, epoch_idx):
        ...
        #alternately train the encoder and the generator 
