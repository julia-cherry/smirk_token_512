...
class SPADE(nn.Module):
    def __init__(self, norm_nc, label_nc):
        ...
        use_bias = True
        self.mlp_token_shared = nn.Sequential(
            nn.Linear(512, nhidden, bias=use_bias),            
            nn.ReLU()
        )
        self.mlp_gamma_token = nn.Linear(nhidden, norm_nc, bias=use_bias)    
        self.mlp_beta_token = nn.Linear(nhidden, norm_nc, bias=use_bias) 

    def forward(self, x, segmap, token):
        ...
        actv = self.mlp_token_shared(token)
        gamma_token = self.mlp_gamma_token(actv)
        beta_token = self.mlp_beta_token(actv)
        gamma_token = gamma_token.view(*gamma_token.size()[:2],1,1)
        beta_token = beta_token.view(*beta_token.size()[:2],1,1)
        out = out * (1 + gamma_token) + beta_token
        return out
...
class SPADEResnetBlock(nn.Module):
    ...
    def forward(self, x, seg1, token):
        x_s = self.shortcut(x, seg1, token)
        dx = self.conv_0(self.actvn(self.norm_0(x, seg1, token)))
        dx = self.conv_1(self.actvn(self.norm_1(dx, seg1, token)))
        out = x_s + dx
        return out

    def shortcut(self, x, seg1, token):
        if self.learned_shortcut:
            x_s = self.conv_s(self.norm_s(x, seg1, token))
        else:
            x_s = x
        return x_s
